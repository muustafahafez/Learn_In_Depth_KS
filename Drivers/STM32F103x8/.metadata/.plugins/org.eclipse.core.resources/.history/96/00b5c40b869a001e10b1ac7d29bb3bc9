...
/*
 * read PINA4->ACC_State:
 *
 * 	if (ACC_State == 1 ) pressed:
 * 		Check (if Traffic Light == Green)
 * 			Function work (give semaphore)
 * 	else
 * 		Do nothing
 * */
...
uint16_t SAFE_DISTANCE =20;//cm

/*-----------------------------*/
//		G_Var_Speed
/*-----------------------------*/
uint16_t CURRENT_DISTANCE;
uint8_t EGO_SET_SPEED=80;
float EGO_SPEED;
...
int main(void)
{
	...
	//Enable IRQs
	//	if(HAL_CAN_ActivateNotification(&hcan, CAN_IT_TX_MAILBOX_EMPTY|CAN_IT_RX_FIFO0_FULL|CAN_IT_RX_FIFO0_MSG_PENDING))
	//	{
	//		Error_Handler();
	//	}
	//Start CAN
